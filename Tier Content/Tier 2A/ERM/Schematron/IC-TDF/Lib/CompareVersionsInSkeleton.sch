<?xml version="1.0" encoding="UTF-8"?><!-- 
    ...
    themselves and descendents of tdf:HandlingAssertion or tdf:StatementMetadata elements.
     ...
     ruleID    := the rule name that invokes the abstrat rule, for use in error message
                  i.e. IC-TDF-ID-00046 
--><sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron" abstract="true" id="CompareVersionsInSkeleton">   
    ...
    <sch:rule id="CompareVersionsInSkeleton-R1" context="$context">
        
        <sch:assert test="every $ver in               (//tdf:*/@*[local-name()=$attrName and namespace-uri()=$namespace]              | //tdf:HandlingAssertion//@*[local-name()=$attrName and namespace-uri()=$namespace]              | //tdf:StatementMetadata//@*[local-name()=$attrName and namespace-uri()=$namespace])              satisfies (number(if (contains($ver,'-')) then substring-before($ver,'-') else $ver))=(number(if (contains(@*[local-name()=$attrName and namespace-uri()=$namespace],'-')) then substring-before(@*[local-name()=$attrName and namespace-uri()=$namespace],'-') else @*[local-name()=$attrName and namespace-uri()=$namespace]))" flag="error" role="error">
            [<sch:value-of select="$ruleID"/>][Error] 
            The {<sch:value-of select="$namespace"/>}<sch:value-of select="$attrName"/>
            ...
            Versions found: <sch:value-of select="string-join(distinct-values(for $ver in                   (//tdf:*/@*[local-name()=$attrName and namespace-uri()=$namespace]                  | //tdf:HandlingAssertion//@*[local-name()=$attrName and namespace-uri()=$namespace]                  | //tdf:StatementMetadata//@*[local-name()=$attrName and namespace-uri()=$namespace])                  return $ver), ', ')"/>
        </sch:assert>
    </sch:rule>
</sch:pattern>