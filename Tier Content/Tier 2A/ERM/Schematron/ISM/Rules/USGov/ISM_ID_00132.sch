<?xml version="1.0" encoding="UTF-8"?><?ICEA pattern?><!-- Notices - Distribution Notice: 
    ...
    <sch:p class="ruleText">[ISM-ID-00132][Error] If ISM_USGOV_RESOURCE and the
        ...
        that have explicit Release specified must have RELIDO. </sch:p>
    ...
    <sch:rule id="ISM-ID-00132-R1" context="*[$ISM_USGOV_RESOURCE         and generate-id(.) = generate-id($ISM_RESOURCE_ELEMENT)         and util:containsAnyOfTheTokens(@ism:disseminationControls, ('RELIDO'))]">
        <sch:assert test="every $ele in $partTags satisfies             if  ($ele/@ism:classification[normalize-space()='U']                  and not(util:containsAnyOfTheTokens($ele/@ism:disseminationControls, ('REL','NF','DISPLAYONLY')))                 and not(util:containsAnyOfTheTokens($ele/@ism:nonICmarkings, ('SBU-NF', 'LES-NF'))))             then true()             else util:containsAnyOfTheTokens($ele/@ism:disseminationControls, ('RELIDO'))" flag="error" role="error"> [ISM-ID-00132][Error] USA documents having RELIDO at the resource level
            must have every classified portion having RELIDO and on any U portions that have
            explicit Release specified must have RELIDO. </sch:assert>
    </sch:rule>
</sch:pattern>