<?xml version="1.0" encoding="UTF-8"?><?ICEA pattern?><!-- Notices - Distribution Notice: 
    ...
    <sch:p class="ruleText">[ISM-ID-00379][Error] All ISM declassDate attributes must be a Date
        without a timezone.</sch:p>
    
    <sch:p class="codeDesc">For all elements which contain a declassDate attribute, this rule ensures that
    ...
    <sch:rule id="ISM-ID-00379-R1" context="*[@ism:declassDate]">
        <sch:assert test="util:meetsType(@ism:declassDate, $DatePattern)" flag="error" role="error">
            [ISM-ID-00379][Error] All declassDate attribute values must be of type Date. </sch:assert>
        <sch:assert test="matches(@ism:declassDate, '[0-9]{4}-[0-9]{2}-[0-9]{2}$')" role="">
