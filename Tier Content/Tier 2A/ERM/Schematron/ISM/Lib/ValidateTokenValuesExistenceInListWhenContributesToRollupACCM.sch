...
--><sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron" abstract="true" id="ValidateTokenValuesExistenceInListWhenContributesToRollupACCM">
    ...
    <sch:rule id="ValidateTokenValuesExistenceInListWhenContributesToRollupACCM-R1" context="$context">
        ...
        <sch:assert test="if (not($contributesToRollup)) then every $searchTerm in tokenize(normalize-space(string($nonACCMSearchTermList)), ' ') satisfies             $searchTerm = $nonACCMListTok or (some $Term in $nonACCMListTok satisfies (matches(normalize-space($searchTerm), concat('^', $Term ,'$')))) else true()" flag="error" role="error">
            <sch:value-of select="$excludedFromRollUpNonACCMErrorMessage"/>
            The value(s) [<sch:value-of select="string-join(for $searchTerm in tokenize(normalize-space(string($nonACCMSearchTermList)), ' ')                  return if($searchTerm = $nonACCMListTok) then null else $searchTerm,' ')"/>] that contribute to rollup could not be found.
        </sch:assert>
    </sch:rule>
</sch:pattern>